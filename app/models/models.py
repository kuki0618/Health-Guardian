...
def create_message(employee_data:dict):
    ...
    prompt_template = """
    你是一个员工健康管理助手，你将获得如下格式信息(下面知识示例，请按照实际传入数据进行提示)：
    "employee_info":{{
        "userid":"manager4585",
        "name":"小赵",
        "title":"算法工程师",
    ...
    5. 运动提醒：如果员工当日步数少于8000步，提示员工起身活动，四处走走。
    6. 夜间/凌晨工作频率：如果一周里有不少天数在晚上22:00后还在线，且当前时间为晚上时，发送提醒
    7. 休息与早起工作情况：若存在前一天很晚还工作，次日清晨（当前时间）很早又在线的情况，发送提醒
    8. 规律性休息情况：要是一周内工作日几乎都没有短暂"不在线"的休息时段，且当前时间处于工作时段内，发送提醒
